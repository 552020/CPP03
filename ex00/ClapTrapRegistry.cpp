...
ClapTrapRegistry::ClapTrapRegistry()
    : _registry(new ClapTrapRegistryEntry[10]), _totalSize(10), _usedSize(0){};
// Destructor
ClapTrapRegistry::~ClapTrapRegistry() { delete[] _registry; };
// Copy constructor
ClapTrapRegistry::ClapTrapRegistry(const ClapTrapRegistry &source)
    : _registry(new ClapTrapRegistryEntry[source._totalSize]),
      _totalSize(source._totalSize), _usedSize(source._usedSize) {
...
void ClapTrapRegistry::addClapTrap(const std::string &name,
                                   ClapTrap *instance) {
  if (_usedSize >= _totalSize) {
    ClapTrapRegistryEntry *newRegistry =
        new ClapTrapRegistryEntry[_totalSize + 10];
...
int ClapTrapRegistry::getUsedSize() const { return _usedSize; }
int ClapTrapRegistry::getTotalSize() const { return _totalSize; }
void ClapTrapRegistry::setUsedSize(int usedSize) { _usedSize = usedSize; }
void ClapTrapRegistry::setTotalSize(int totalSize) { _totalSize = totalSize; }
